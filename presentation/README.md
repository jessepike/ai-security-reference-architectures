...
The PowerPoint is an editable teaching sequence built from the architecture and guide Markdown in `../content/`. Story v0.3 explains how governance sets the conditions for using AI, how security puts those conditions into practice, and how both respond to what happens in use. `slide-plan.md` maps that sequence to its sources. `build-deck.mjs` contains the slide copy and editable layout instructions; `finalize-deck.mjs` validates the exported package. `generate-manuscript.py` regenerates the human-readable `manuscript.md` from the candidate PPTX.
