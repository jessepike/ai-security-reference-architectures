...
The PowerPoint is an editable teaching sequence built from the architecture and guide Markdown in `../content/`. `slide-plan.md` maps that sequence to its sources. `build-deck.mjs` contains the slide copy and editable layout instructions; `finalize-deck.mjs` validates the exported package. The exported `manuscript.md` provides a human-readable review copy of slide text and speaker notes.
...
The four PNG architecture views are source images, so their internal text is not editable in PowerPoint. The other teaching diagrams, headings and body text use native editable presentation elements. Each PNG also has an accessible Markdown visual contract.
